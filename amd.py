...
# ROCm is not supported on Windows (Ubuntu full-support)
def rocm_is_supported():
...
def main(use_refiner: bool, use_cpu: bool):
    ...
    if use_cpu:
        ...
        base.to(torch_directml.device(), non_blocking=True)
    ...
    if use_refiner:
        ...
        if use_cpu:
            ...
            # Use DirectML for AMD GPU
            refiner.to(torch_directml.device(), non_blocking=True)
    ...
    if use_refiner:
        ...
        image = image[0]
    # Save image
    save_image(image, file_name, format="png", normalize=False)
...
if __name__ == '__main__':
    ...
    main(use_refiner=len(refiner_name) > 0, use_cpu=rocm_is_supported())